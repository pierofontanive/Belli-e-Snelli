...
app.get('/api/esercizi', (req, res) => {
    mongoose.connect(CONNECTION_STRING, { useNewUrlParser: true, useUnifiedTopology: true }, (error,client) => {
        var dbt = mongoose.connection;
        dbt.once("open", () => console.log("Collegato con successo al database"));
        dbt.collection("Esercizi").find({}).toArray((error, result) => {
...
app.get('/api/prenotazioni', (req,res) => {
    mongoose.connect(CONNECTION_STRING, { useNewUrlParser: true, useUnifiedTopology: true }, (error,client) => {
        var dbt = mongoose.connection;
        dbt.once("open", () => console.log("Collegato con successo al database"));
        dbt.collection("Prenotazioni").find({}).toArray((err,result) => {
